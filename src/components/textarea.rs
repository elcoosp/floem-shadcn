...
impl IntoView for Textarea { type V = Box<dyn View>;
    ...
    fn into_view(self) -> Self::V { Box::new(floem::views::TextInput::new(RwSignal::new(String::new())).style(|s| s.width_full().border_radius(6.0).border(1.0).padding_left(12.0).padding_right(12.0).padding_top(8.0).padding_bottom(8.0).font_size(14.0).with_shadcn_theme(|s,t|{ let ring=t.ring; s.border_color(t.input).background(t.background).color(t.foreground).focus(move |s| s.outline(2.0).outline_color(ring)) }))) } }
