...
use floem::{
    Renderer, View, ViewId, action::exec_after,
...
impl TextInput {
    ...
    pub fn with_text_and_id(text: impl Into<String>, id: ViewId) -> Self {
        ...
        Effect::new(move |_| {
            ...
            if width <= 0.0 { return; }
            let new_offset = if cursor_x < current_offset { cursor_x }
                else if cursor_x > current_offset + width { cursor_x - width }
                else { current_offset };
            if new_offset != current_offset { scroll_offset.set(new_offset); }
        });
        ...
        id.add_event_listener(EventListener::KeyDown, Box::new(move |event| {
            ...
            if key == &Key::Named(NamedKey::Enter) {
                on_enter.with_untracked(|cb| { if let Some(c) = cb { let text = doc_signal.get_untracked().text(); c(&text); } });
            ...
            if let Some(command) = command {
                ...
                match command {
                    Command::Edit(edit_cmd) => { document.run_edit_command(edit_cmd); id.request_layout(); }
                    Command::Move(move_cmd) => { document.run_move_command(move_cmd, shift_held); id.request_paint(); }
                    Command::SelectAll => { document.select_all(); id.request_paint(); }
                    Command::Copy => { document.copy(); }
                    Command::Cut => { if document.cut() { id.request_layout(); } }
                    Command::Paste => { if document.paste(true) { id.request_layout(); } }
                }
                last_cursor_action.set(Instant::now());
                return EventPropagation::Stop;
            }
    ...
    pub fn on_update(self, on_update: impl Fn(&str) + 'static) -> Self { self.doc.get_untracked().on_update(on_update); self }
    pub fn placeholder(self, text: impl Into<String>) -> Self { self.placeholder.set(Some(text.into())); self }
    pub fn on_enter(self, callback: impl Fn(&str) + 'static) -> Self { self.on_enter.set(Some(Box::new(callback))); self }
    pub fn value(self, set_value: impl Fn() -> String + 'static) -> Self {
        ...
        Effect::new(move |_| {
            ...
            if current_text == filtered { return; }
            doc.with_untracked(|doc| {
                ...
                doc.edit([(SelRegion::new(0, end, CursorAffinity::Forward, None), filtered.as_str())], EditType::Other);
            });
        });
        self
    }
...
impl View for TextInput {
    ...
    fn compute_layout(&mut self, _cx: &mut ComputeLayoutCx) -> Option<Rect> {
        ...
        if width != self.visible_width.get_untracked() { self.visible_width.set(width); }
    ...
    fn paint(&mut self, cx: &mut PaintCx) {
        ...
        cx.save();
        cx.clip(&Rect::from_origin_size(Point::new(padding.3, padding.0), Size::new(visible_width, height)));
        let doc = self.doc.get_untracked();
        let text = doc.text();
        let lines = doc.text_layouts().borrow();
        ...
        if text.is_empty() {
            if let Some(placeholder_text) = self.placeholder.get_untracked() {
                let mut attrs = floem::text::Attrs::default()
                    .font_size(text_styles.font_size).color(placeholder_color)
                    .line_height(text_styles.line_height).weight(text_styles.font_weight);
                if !text_styles.font_family.is_empty() { attrs = attrs.family(&text_styles.font_family); }
                let pl = floem::text::TextLayout::new_with_text(&placeholder_text, floem::text::AttrsList::new(attrs), None);
            ...
            cx.draw_text_with_layout(layout_iter, Point::new(padding.3 - scroll_offset, padding.0 + y_offset));
        }
        if cx.is_focused(self.id) {
            ...
            if cursor.is_caret() {
                ...
                if is_cursor_visible(elapsed) {
                    ...
                    let rect = Rect::from_origin_size((p.x + padding.3 - scroll_offset - 1.0, ct + padding.0 + y_offset), (2.0, ch));
                ...
                let (st, sh) = get_glyph_dimensions(p.glyph_top, p.glyph_bottom, lines.default_glyph_top(), lines.default_glyph_height());
                let rect = Rect::from_origin_size((sx + padding.3 - scroll_offset, st + padding.0 + y_offset), (ex - sx, sh));
