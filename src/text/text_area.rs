...
use floem::{
    IntoView, Renderer, View, ViewId, action::exec_after,
...
#[cfg(test)] use super::KeyPress;
#[cfg(test)] use ui_events::keyboard::NamedKey;

const RESIZE_HANDLE_SIZE: f64 = 16.0;

pub struct TextArea {
    id: ViewId, scroll_id: ViewId, dummy_child_id: ViewId,
    doc: RwSignal<Document>,
    padding: RwSignal<(f64, f64, f64, f64)>,
    viewport: RwSignal<Rect>, parent_size: RwSignal<Size>,
...
impl TextArea {
    ...
    pub fn with_text_and_id(text: impl Into<String>, id: ViewId) -> Self { Self::with_text_id_and_keymap(text, id, Keymap::multi_line()) }
    pub fn with_text_id_and_keymap(text: impl Into<String>, id: ViewId, keymap: Keymap) -> Self {
        ...
        let scroll_view = Scroll::new(dummy_child)
            .style(move |s| { let p = padding.get(); s.absolute().size_full().margin_top(-p.0).margin_left(-p.3); })
            .on_child_size(move |s| { scroll_child_size.set(s); })
            .ensure_visible(move || {
                ...
                Rect::from_origin_size((0.0, pt.line_top), (1.0, pt.line_bottom - pt.line_top + padding.0 + padding.2))
            })
            .on_scroll(move |v| { viewport.set(v); });
        ...
        id.add_event_listener(EventListener::PointerDown, Box::new(move |event| {
            if let Event::Pointer(PointerEvent::Down(pe)) = event {
                ...
                if resize_enabled.get_untracked() {
                    ...
                    let w = layout.size.width as f64; let h = layout.size.height as f64;
                ...
                id.request_active(); id.request_focus();
        ...
        id.add_event_listener(EventListener::PointerMove, Box::new(move |event| {
            if let Event::Pointer(PointerEvent::Move(pe)) = event {
                if is_resizing.get_untracked() {
                    ...
                    let dx = cp.x - sp.x; let dy = cp.y - sp.y;
                    let mut nw = (ss.width + dx).max(min.width);
                    let mut nh = (ss.height + dy).max(min.height);
                    if let Some(mx) = max { nw = nw.min(mx.width); nh = nh.min(mx.height); }
        ...
        id.add_event_listener(EventListener::PointerUp, Box::new(move |event| {
            if let Event::Pointer(PointerEvent::Up(pe)) = event {
                if is_resizing.get_untracked() { is_resizing.set(false); return EventPropagation::Stop; }
                let padding = padding.get_untracked();
                let vp = viewport.get_untracked();
                let mut adj = pe.clone();
                adj.state.position.x -= padding.3;
        ...
        id.add_event_listener(EventListener::KeyDown, Box::new(move |event| {
            let Event::Key(KeyboardEvent { state: KeyState::Down, key, modifiers, .. }) = event else { return EventPropagation::Continue; };
            let command = keymap_clone.get(key, modifiers);
            let document = doc_signal.get_untracked();
            if let Some(command) = command {
                let shift_held = modifiers.shift();
                match command {
                    Command::Edit(ec) => { document.run_edit_command(ec); id.request_layout(); }
                    Command::Move(mc) => { document.run_move_command(mc, shift_held); scroll_id.request_layout(); }
                    Command::SelectAll => document.select_all(),
                    Command::Copy => { document.copy(); }
                    Command::Cut => { if document.cut() { id.request_layout(); } }
                    Command::Paste => { if document.paste(false) { id.request_layout(); } }
                }
                last_cursor_action.set(Instant::now());
                return EventPropagation::Stop;
            }
            ...
            #[cfg(target_os="macos")] mods.set(Modifiers::ALT, false);
            if mods.is_empty() {
                if !matches!(key, Key::Character(_)) { return EventPropagation::Continue; }
                if let Key::Character(c) = key { document.insert_text(c); id.request_layout(); last_cursor_action.set(Instant::now()); }
            }
            EventPropagation::Stop
        }));

        id.add_event_listener(EventListener::ImeCommit, Box::new(move |event| {
            if let Event::ImeCommit(text) = event { doc_signal.get_untracked().insert_text(text); id.request_layout(); last_cursor_action.set(Instant::now()); }
            EventPropagation::Stop
        }));

        Self { id, scroll_id, dummy_child_id, doc: doc_signal, padding, viewport, parent_size, child_height, last_cursor_action, resize_enabled, resize_size, is_resizing, resize_start_pos, resize_start_size, min_size, max_size }
    }
    pub fn doc(&self) -> RwSignal<Document> { self.doc }
    pub fn scroll_id(&self) -> ViewId { self.scroll_id }
    pub fn child_height(&self) -> RwSignal<f64> { self.child_height }
    pub fn viewport(&self) -> RwSignal<Rect> { self.viewport }
    ...
    pub fn value(self, set_value: impl Fn() -> String + 'static) -> Self {
        ...
        Effect::new(move |_| {
            ...
            doc.with_untracked(|d| {
                ...
                use floem_editor_core::{cursor::CursorAffinity, editor::EditType, selection::SelRegion};
                d.edit([(SelRegion::new(0, end, CursorAffinity::Forward, None), nv.as_str())], EditType::Other);
            });
        });
        self
    }
    pub fn text(&self) -> String { self.doc.get_untracked().text() }
    pub fn resizable(self, enabled: bool) -> Self { self.resize_enabled.set(enabled); self }
    pub fn is_resizable(&self) -> bool { self.resize_enabled.get_untracked() }
    pub fn resize_size(&self) -> RwSignal<Option<Size>> { self.resize_size }
    pub fn min_resize_size(self, size: Size) -> Self { self.min_size.set(size); self }
    pub fn max_resize_size(self, size: Size) -> Self { self.max_size.set(Some(size)); self }
    pub fn is_resizing(&self) -> bool { self.is_resizing.get_untracked() }
}
...
impl View for TextArea {
    ...
    fn view_style(&self) -> Option<Style> {
        ...
        Some(Style::new().cursor(StyleCursorStyle::Text).focusable(true)
             ...
             .apply_if(rs.get().is_some(), move |s| { let sz = rs.get().unwrap(); s.width(sz.width).height(sz.height) }))
    }
    fn compute_layout(&mut self, cx: &mut ComputeLayoutCx) -> Option<Rect> {
        ...
        if padding != self.padding.get_untracked() { self.padding.set(padding); }
        ...
        if child_height != self.child_height.get_untracked() {
            ...
            let mut sty = self.id.taffy().borrow().style(node).cloned().unwrap_or_default();
            sty.size.height = Dimension::length(child_height as f32);
            self.id.set_taffy_style(node, sty);
        }
        if self.parent_size.get_untracked() != parent_size { self.parent_size.set(parent_size); }
    ...
    fn paint(&mut self, cx: &mut PaintCx) {
        ...
        cx.save();
        cx.clip(&self.parent_size.get_untracked().to_rect().with_origin(Point::new(padding.3, padding.0)).inflate(2.0, 0.0));
        let doc = self.doc.get_untracked();
        let lines = doc.text_layouts().borrow();
        ...
        if cx.is_focused(self.id) {
            ...
            if cursor.is_caret() {
                let elapsed = self.last_cursor_action.get_untracked().elapsed().as_millis();
                if is_cursor_visible(elapsed) {
                    let p = lines.point_of_offset(cursor.end);
                    let (ct, ch) = get_glyph_dimensions(p.glyph_top, p.glyph_bottom, lines.default_glyph_top(), lines.default_glyph_height());
                    let rect = Rect::from_origin_size((p.x + padding.3 - 1.0, ct + padding.0 - viewport.y0), (2.0, ch));
                    cx.fill(&rect, text_styles.text_color, 0.0);
                }
                let id = self.id;
                exec_after(Duration::from_millis(CURSOR_BLINK_INTERVAL_MS), move |_| { id.request_paint(); });
            } else {
                ...
                for line in lines.visual_lines(so..eo+1) {
                    let x0 = if line.line_i == sv { lines.point_of_offset(cursor.min()).x } else { 0.0 };
                    let x1 = if line.line_i == ev { lines.point_of_offset(cursor.max()).x } else { line.line_w as f64 };
                    let rect = Rect::from_origin_size((x0+padding.3, line.line_top as f64 + padding.0 - viewport.y0), (x1-x0, line.line_height as f64));
        ...
        if self.resize_enabled.get_untracked() {
            ...
            let ds = 3.0; let dot = 2.0; let bx = w-4.0; let by = h-4.0;
            for row in 0..3 { for col in 0..=row { let x = bx - row as f64*ds + col as f64*ds; let y = by - col as f64*ds; cx.fill(&Rect::from_origin_size((x-dot/2., y-dot/2.), (dot,dot)), hc, 0.0); } }
        }
    }
}
