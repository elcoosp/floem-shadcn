...
fn test_item_renders_title_and_description() {
 ...
 let mut harness = HeadlessHarness::new_with_size(TestRoot::new(), TestRoot::new(), TestRoot::new(), container, 400.0, 100.0);
 harness.rebuild();

 let layout = id.get_layout().expect("Item layout should exist");
 assert!(layout.size.width > 50.0, "Item should have width");
 assert!(layout.size.height > 40.0, "Item should be tall enough for title+description");
}
